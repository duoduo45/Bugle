...
@interface ContactEditViewController () <ContactModelObserver>
{
...
- (IBAction)takeAPicture:(id)sender 
{
    ...
    self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:@"Save" style:UIBarButtonItemStylePlain target:self action:@selector(submit:)] autorelease];
...
- (void)setID:(NSString*)userid setFirstName:(NSString*)firstname setLastName:(NSString*)lastname setEmail:(NSString*)email setPhone:(NSString*)phone setNotes:(NSString*)notes setImageURL:(NSString*)imageURL
{
    ...
    if (infoArray != nil)
        infoArray = nil;
    infoArray = [[NSArray alloc] initWithObjects:email, phone, notes, nil];
    
    CGRect frame = CGRectMake(10, 35, 87, 87);
    frame.origin.y += 44;
    
    if (imageURL.length > 0)
    {
        InternetImageView *iv = [[InternetImageView alloc] initWithFrame:frame];
        [iv imageFromURL:imageURL Delegate:self];
...
- (void)addNewContact 
{
    ...
    newContact.photoData = UIImagePNGRepresentation([self.photoButton.imageView.image cropImage:self.photoButton.imageView.image]);
    
    for( int i=0; i<5; i++ )
    {
        [[textArray objectAtIndex:i] resignFirstResponder];
    }
    
    [_contactModel editContact:newContact];
    [newContact release];
}
...
- (void)viewDidLoad
{
    ...
    contactTable = [[UITableView alloc] initWithFrame:CGRectMake(10, CGRectBottom(self.photoButton.frame)+44, 300, 228) style:UITableViewStylePlain];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath 
{
    ...
    else if (tableView == contactTable)
    {
        ...
        if (indexPath.row == 2)
        {
            ...
            infoContent.text = [infoArray objectAtIndex:indexPath.row];
...
- (void)contactModel:(ContactModel *)contactModel editContacts:(ReturnParam *)param
{
    ...
    if( param.success )
    {
        [((ContactListViewController*)(self.rootViewController)) refreshProfile];
